...
li.innerText = `${info[0]}`
li1.innerText = `${info[1]}`
li2.innerText = `${info[2]}`
li3.innerText = `${info[3]}`
...
for (const courses of coursesAndDurationArray) {
    ...
    heading.innerText = `title: ${courses.title}`;

    let description = document.createElement(`p`);
    description.classList.add(`description`);
    description.innerText = `monthDuration: ${courses.monthDuration}`;
...
for (const simpson of simpsons) {
    ...
    h2.innerText = `Персонаж - ${simpson.name} ${simpson.surname},   ${simpson.age}`;
...
for (const course of coursesArray) {
    ...
    p2.innerText = `Hour Duration - ${course.hourDuration}`;
